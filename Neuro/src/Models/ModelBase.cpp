    ...
    void ModelBase::InitGraph(const vector<TensorLike*>& inputs, const vector<TensorLike*>& outputs)
    {
        ...
        for (size_t i = 0; i < inputs.size(); ++i)
        {
            ...
            NEURO_ASSERT(layer->m_InboundNodes.size() == 1 && (layer->m_InboundNodes.empty() || layer->m_InboundNodes[0]->inbound_layers.size() == 0),
                ClassName() + " inputs must come from `Input` (thus holding past layer metadata), they cannot be the output of a previous non-Input layer. Here, a tensor specified as input to your model was not an Input tensor, it was generated by layer " <<
        ...
        // we have to add new inbound node because sequencial model can be shared in its current state before adding more layers
