...
//////////////////////////////////////////////////////////////////////////
TensorLike* FastNeuralStyleTransfer::CreateTransformerNet(TensorLike* input)
{
    ...
    auto convLayer = [&](TensorLike* input, uint32_t filtersNum, uint32_t filterSize, uint32_t stride = 1, bool doUpSample = false, bool doNorm = true, bool doReLU = true)
    {
        ...
        if (doNorm)
            input = (new BatchNormalization())->Call(input)[0];
        if (doReLU)
            input = relu(input);
        return input;
    };
...
//////////////////////////////////////////////////////////////////////////
ModelBase* FastNeuralStyleTransfer::CreateGeneratorModel(uint32_t width, uint32_t height)
{
    ...
    auto residual_block = [&](TensorLike* x, int num)
    {
        ...
        x = (new InstanceNormalization("resi_normal_" + to_string(num) + "_1"))->Call(x)[0];
        ...
        x = (new InstanceNormalization("resi_normal_" + to_string(num) + "_2"))->Call(x)[0];
    ...
    c1 = (new InstanceNormalization("normal_1"))->Call(c1)[0];
    ...
    c2 = (new InstanceNormalization("normal_2"))->Call(c2)[0];
    ...
    c3 = (new InstanceNormalization("normal_3"))->Call(c3)[0];
    ...
    d1 = (new InstanceNormalization("normal_4"))->Call(d1)[0];
    ...
    d2 = (new InstanceNormalization("normal_5"))->Call(d2)[0];
    ...
    c4 = (new InstanceNormalization("normal_6"))->Call(c4)[0];
